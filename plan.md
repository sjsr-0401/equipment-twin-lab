...
```text
현재 프로젝트는 실제 장비 안전 인증이 아니라,
장비 SW Core의 상태 전이, 가상 IO, Timeout, 알람/복구 시나리오를
반복 가능하게 검증하는 소프트웨어 모델이다.
```
...
- 알람 코드 체계는 다음 Goal 후보로 남긴다.
- 실제 PLC/Unity/카메라 연결은 아직 확정하지 않는다.
...
## 39. 2026-06-30 Goal 014: Motion Axis 모델

PR #11을 병합하고 Goal 014를 시작했다.

사용자가 말한 큰 목표:

```text
사용자가 장비, 제품, 공정, 검사 조건, 트러블 조건을 선택해서
실제처럼 제조/검사 시뮬레이션하는 프로그램
```

판단:

- 방향은 좋다.
- 다만 30일 안에는 자유 배치형 장비 빌더보다 “정해진 모듈을 조합하는 Configurable Manufacturing Digital Twin”이 현실적이다.
- 첫 모듈은 `MotionAxis`가 맞다. 큰 장비도 결국 축 단위 Servo/Home/Move/InPosition/Alarm 패턴을 공유하기 때문이다.
...
- 아직 공정 상태와 모션 축을 연결하지 않았다.
- 아직 Scenario JSON action으로 모션을 실행하지 않는다.
- 아직 Unity와 연결하지 않는다.
