...
## 2026-07-01 이해 요약: Demo Rehearsal Runner

이번 Goal의 핵심은 “영상을 찍기 전에 데모가 깨지지 않는지 자동으로 확인하는 것”이다.

한 문장 설명:

> `Invoke-PortfolioDemoRehearsal.ps1`은 build/test/정상 공정/fault 공정/Unity screenshot을 순서대로 실행하고, 녹화용 report를 `artifacts/demo-rehearsal/recording-rehearsal.md`에 남기는 스크립트다.

코드 흐름:

```text
Invoke-PortfolioDemoRehearsal.ps1
    -> Invoke-DemoStep("Release build")
    -> Invoke-DemoStep("Core console tests")
    -> Invoke-DemoStep("Normal public moly ALD process")
    -> Invoke-DemoStep("Expected pumpdown timeout fault", AllowedExitCodes = 1)
    -> Invoke-DemoStep("Unity screenshot capture")
    -> Write-RehearsalReport()
```

중요한 개념:

- fault demo는 실패가 정상일 수 있다.
- `pumpdown-timeout`은 장비 공정이 의도적으로 `Alarmed`로 멈추는 것을 보여준다.
- 그래서 exit code `1`을 script failure로 보지 않고 expected result로 처리한다.

막힌 점과 해결:

- PowerShell에서 Markdown backtick은 escape 문자라 syntax error가 날 수 있다.
- Windows PowerShell에는 `Path.GetRelativePath`가 없어 URI 기반 상대 경로 계산을 사용했다.
- Unity custom screenshot path는 한글 경로에서 command-line 전달이 불안정할 수 있어, 기본 screenshot을 만든 뒤 리허설 폴더로 복사한다.
