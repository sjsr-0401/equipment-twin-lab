...
- 단계: 장비 상태머신, 가상 IO 모델, CI, Clock/Timeout 모델, IO-상태 연결 계층, Scenario JSON Runner, Scenario CLI 실행기, CLI batch 리포트, 알람/복구 시나리오, Core 검증 정리 문서, 알람 코드 체계, Visual Studio build/debug 지원, 알람 복구 조건 구현 완료, CLI 리포트 알람/복구 조건 표시 완료, 가상 모션 축 모델 완료, 모션 Scenario JSON action 구현, Equipment Template / Product Recipe 최소 모델 구현, Template Runner 구현, Fault Model 구현, Inspection Result Model 구현, Template Runner CLI 구현
- 루프 방식: 수동 실행
- 자동화 상태: 아직 없음
- 자동 병합 상태: 금지
...
1. Goal 020 변경을 커밋하고 Draft PR을 만든다.
2. CI에서 Release build, console tests 68개, CLI batch 9개, template CLI 2개 실행을 확인한다.
3. 이후 Inspection Scenario Selection 또는 Template Run Markdown Report로 확장한다.
